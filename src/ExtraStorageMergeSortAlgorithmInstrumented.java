 ...
 * 
 *          27 Nov 2006: (bwbecker@uwaterloo.ca) Changed the overall
 *          architecture to use model-view-controller. Re-examined algorithm to
 *          ensure that counting was consistent across all algorithms (the
 *          quicksorts used to ignore most of the comparisons in the
 *          partitioning, for example).
 */
class ExtraStorageMergeSortAlgorithmInstrumented extends SortAlgorithm {
	...
	void sort(int a[], int lo, int hi, int scratch[]) {
		...
		ins.startTiming("merge sort recursion");
		numberOfCalling++;
		...
		if (lo >= hi) {
			ins.stopTiming("merge sort recursion");
			numberOfCalling++;

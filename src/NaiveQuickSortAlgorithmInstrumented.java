...
/**
 * A quick sort demonstration algorithm SortAlgorithm.java, Thu Oct 27 10:32:35
 * 1994
 *
 ...
 *          Misbehaviour expressed when the pivot element was not unique. -Jason
 *          Harrison
 *
 *          21 Jun 1996: Modified code based on comments from Paul Haeberli, and
 *          Peter Schweizer (Peter.Schweizer@mni.fh-giessen.de). Used Daeron
 *          Meyer's (daeron@geom.umn.edu) code for the new pivoting code. -
 *          Jason Harrison
 *
 ...
 *          uses a[hi] as the pivot to show the abysmal performance with a poor
 *          pivot choice. Changed the overall architecture to use
 *          model-view-controller. Re-examined algorithm to ensure that counting
 *          was consistent across all algorithms (the quicksorts used to ignore
 *          most of the comparisons in the partitioning, for example).
 */
class NaiveQuickSortAlgorithmInstrumented extends SortAlgorithm {
	private static int numberOfCalling = 0;

	void sort(int a[], int lo0, int hi0) {
		Instrumentation ins = Instrumentation.Instance();
		ins.startTiming("native quick sort recursion");
		numberOfCalling++;
		int lo = lo0;
		int hi = hi0;
		super.lowMarker = lo0;
		super.hiMarker = hi0;
		if (lo >= hi) {
			ins.stopTiming("native quick sort recursion");
			numberOfCalling++;
			return;
		} else if (lo == hi - 1) {
			/*
	...
	void sort(int a[]) {
		...
		ins.comment("numberOfCalling: " + numberOfCalling);
	}
}